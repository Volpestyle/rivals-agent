...
> **Archived 2026-09-23** from `docs/lanes/local-jev.md`, verbatim below this note. The `localjev/` code it describes
> (`serve.sh`, `paired.py`, `netprobe.py`, `gpustat.py`, `selftest.py`) was deleted from the repo; the last tree with
> it is `7a543e5` (for example `git show 7a543e5:localjev/paired.py`). The Mac's `.localjev/` install and any PC copies
> are outside git and untouched. "Running." below is the 2026-09-20 status; the server is parked
> (`docs/learning-plan.md`), which cites the "Measured" section here for its latency numbers.
