...
for kind in ('controller', 'perception'):
    ...
    data.update(run_limit_s=20, startup_limit_s=14, combined_authorization_s=34,
    ...
    if 'D_comparisons' in data:
        ...
        data['D_delta'] = 'New request-start-owned-pulse-v1 interpretation: fresh starts retain original100ms request/resource deadline; owned nominal33ms pulse may finish after request expiry, capped by phase/session. Exact-output HUD countdown candidate filtering changed. Historical D supplies primitive identity, not new holding-authority or timing equivalence.'
