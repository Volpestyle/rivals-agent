...
LIVE = {"19-53-04": "2026-09-25 19-53-04.mkv", "20-25-52": "2026-09-25 20-25-52.mkv", "20-56-10": "2026-09-25 20-56-10.mkv"}
...
USED = {"19-53-04": (9, 3), "20-25-52": (9, 3), "20-56-10": (9, 3), "daymr-r2r4": (4, 2)}   # first-draw windows decoded
...
def main(out_path):
    ...
    guard = operator_times()
    timer = draw(rng, REPLAY_STRETCHES, 10.0, [], MAX_WINDOWS, guard, avoid=first_windows("daymr-r2r4"))
    feed = draw(rng, REPLAY_STRETCHES, 60.0, [], MAX_WINDOWS, guard, avoid=first_windows("daymr-r2r4"))
    total = sum(b - a for a, b in REPLAY_STRETCHES)
    values = []
    tries = 0
    while len(values) < 100 and tries < 100000:
        tries += 1
        u = rng.uniform(0, total)
        for a, b in REPLAY_STRETCHES:
            if u <= b - a:
                t = round(a + u, 3)
                break
            u -= b - a
        if not any(t - 1.0 <= g <= t + 1.0 for g in guard) and not any(a - MARGIN <= t <= b + MARGIN for a, b in first_windows("daymr-r2r4")):
            values.append(t)
    plan["recordings"]["daymr-r2r4"] = {"video": REPLAY, "stretches": REPLAY_STRETCHES, "timer_windows": timer,
                                        "timer_n": 4, "feed_windows": feed, "feed_n": 2,
                                        "value_frames_s": sorted(values)}
