...
**The motor record.** `motor-settings.json` `8955d5b8` was the session's motor record when I reviewed it, and it is the
sha256 the evidence names. After the motor fix-forward it is kept as `motor-settings.v1.json`. The regenerated
`motor-settings.json` (`2a6e604e`) is re-checked in the `review-intake-motor.md` addendum.
