...
too; each decision runs after its row's updates. Labels (docs/lanes/tracker.md): every box before t 13.8 s is the spawn room's lime glass
...
def replay(mod, no_kill_feed=False):
    ...
    for n, (kind, t, boxes, i) in enumerate(events):
        ...
        if "state" in r:
            ...
            brain.decide(State(t=base.t, frame=base.frame, hp=base.hp, max_hp=base.max_hp, webs=base.webs, abilities=base.abilities,
                               detections=list(use[0]), coasting=use[1]), m)
        tgt = m.target
        ticks.append((r["t"], None if tgt is None else label(m.target_t, tgt.bbox),
