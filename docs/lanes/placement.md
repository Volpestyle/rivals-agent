...
**The kill switch, for every step that sends input** (B, C, D and `--live`):
- **Any physical key or mouse input** (review-lowmap L4, `agent/physical_input.py`).
  - A separate sentinel process registers raw input with `RIDEV_INPUTSINK`, so it hears the devices while the game
    has focus. It trips on a control-affecting packet (the importer's `_control_affecting`) from a real device
    handle; injected packets (handle 0) never trip.
  - Armed, a trip fails the proof before the next 50 ms write and `Live`'s commit guard: release and stop.
  - A hung executor that does not acknowledge the trip within 1 s is terminated by the sentinel, which unplugs the pad.
  - No READY from the sentinel, or a stale heartbeat (> 0.5 s): refused or stopped.
- **The positive pre-run test,** before any pad code is imported:
  - a physical touch must trip a dry run of the same check within 30 s;
  - then the hands are off for 3 s with the game in front (Alt-Tab to it, no click: LMB is an attack);
  - only then does the sentinel arm.

  A failed test means REFUSED, no pad.
- **Alt-Tab, or click any other window.** The game reads the pad only while it has focus. `scripts/place.py` also
  checks focus, the range HUD and the idle banner before every 50 ms write.
- **Ctrl-C** in the agent's terminal closes the pad.
- Nothing below presses a button. Only B, C and D touch a stick, and only the right (camera) stick.
...
| D | **Low-end stick map (K3)** (~2 min). James stands on the spawn plaza, away from any drop, facing textured scenery with no bot near the crosshair, and lets go:<br>`python scripts/place.py --lowmap --declaration <measurement.json>`<br>The measurement declaration must list the mode `lowmap` (and the lead's authorization must name it), like B and C. It must also carry `"pad_settings": {"curve": "Linear", "horizontal": 265, "vertical": 75, "aim_assist": 0}`, or the mode refuses before any pad opens. | The M1 prime and its turn back, as in B. Then, twice each: right stick x at 0.02, 0.04, 0.05, 0.06, 0.07, 0.08, 0.09 and 0.10, each held 1.0 s right then 1.0 s left; right stick y at 0.05, 0.1, 0.15, 0.2, 0.3, 0.4 and 0.5, each held 0.5 s up then 0.5 s down. A settled frame (0.35 s) before and after every hold. Right stick only: the hold refuses any other key. | The camera turns slowly or not at all, and ends near where it started. Never a step, never a translation.<br>The rotation over each hold is measured from the settled frames by `perception/camera_motion.py`, and the rate uses the stick's measured on-time. It writes `data/placement/lowmap-<stamp>.json` (Cal's `yaw_map`, `pitch_map`, `yaw_deadzone`, `pitch_deadzone`; the deadzone is the largest still deflection, with its edge) and the frames under `data/placement/lowmap-<stamp>/`. It prints `ok` and `why`.<br>It is not `ok` on:<br>- an abstaining fit;<br>- a turn against the stick;<br>- a deflection that moved on some holds only;<br>- forward and back, or the two repeats, disagreeing by more than max(25 %, 0.5°) (review L1);<br>- off-axis rotation over max(the still threshold, 20 % of the on-axis) (L2);<br>- a still deflection above a moving one;<br>- no movement at all;<br>- a top rate more than 25 % off Cal's map (other pad settings).<br>Only an `ok` file carries `cal`; a not-ok one carries `cal_candidate` (L3). `python scripts/place.py --lowmap-judge FILE [--out FILE]` re-judges a saved file from its raw rows, offline, and never rewrites it. The rate uses the stick's on-time, from the first send to the release.<br>Tests: `tests/test_place_lowmap.py` (synthetic rendered frames), `tests/test_physical_input.py`. Input-safety review: `review-lowmap.md` (safe as written); L1–L4 re-check pending. |
...
- **From D:** the deadzone and low-end map, taken from `lowmap-<stamp>.json` into `agent.controller.Cal` only when it says `ok`.
