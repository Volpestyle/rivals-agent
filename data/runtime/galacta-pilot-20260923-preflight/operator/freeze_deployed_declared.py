"""Re-freeze for galacta-pilot-20260923 with a lead-declared runtime change; issues no live authority.

Operator copy of the tracked freeze_deployed.py (kept outside the repo: the live tree reaches data/ through a junction).
Difference: --allow names the only runtime paths that may differ from the identity commit (the lead-declared scorer-
evidence reader fix); they must leave perception/selector identity unchanged, are added to the perception manifest's
pinned files, and are recorded under declared_runtime_changes with their commits and --note.
...
import os
ROOT = Path(os.environ.get("PILOT_TEST_ROOT", "C:/Users/volpe/repos/rivals-agent"))   # override only for the scratch test
HERE = ROOT / "data/runtime/galacta-pilot-20260923-preflight"
...
def main(argv=None):
    ...
    ap.add_argument("--allow", action="append", required=True, help="declared runtime path")
    ...
    unexpected = sorted(set(changed) - set(a.allow))
    if unexpected or not changed:
        refuse(f"runtime changes since {cand['identity_commit']}: {changed}; declared {a.allow}; undeclared {unexpected}")
    ident_paths = {p for k in ("perception", "selector") for p in cand["identity_files"][k]}
    if ident_paths & set(changed):
        refuse(f"a declared change touches identity files: {sorted(ident_paths & set(changed))}")
    declared = {"paths": sorted(changed), "note": a.note, "since": cand["identity_commit"],
                "commits": git(live, "log", "--format=%H %s", f"{cand['identity_commit']}..{head}", "--", *changed).decode().splitlines()}
    ...
    for kind in ("controller", "perception"):
        ...
        data.update(status="deployed_bytes_equal_main_git_blobs_up_to_CRLF", checkout=str(live), git_commit=head,
                    ...
                    files={p: pin(p) for p in [*data["files"], *(declared["paths"] if kind == "perception" else [])]},
        ...
        if kind == "perception":
            data["declared_runtime_changes"] = {**declared, "pins": {p: pin(p) for p in declared["paths"]}}
    ...
    write("deployment-check.json", {"commit": head, "main": main_ref, "predeclared_commit": cand["predeclared_commit"],
                                    "identity_commit": cand["identity_commit"], "runtime_paths_unchanged": False,
                                    "declared_runtime_changes": declared,
