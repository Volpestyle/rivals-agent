...
MOTOR_SETTINGS = ("dpi", "horizontal_sensitivity", "vertical_sensitivity", "swing_mode")
...
def ui_key_presses(raw_events):
    """[(t_ns, vk)] for every key-down packet of a UI key (repeats included: each is a press packet)."""
    return [(int(e["t_ns"]), int(e["vk"])) for e in raw_events if e.get("type") == "key" and e.get("down")
            and int(e["vk"]) in UI_KEYS]
...
def _cuts(a, b, *, ui_keys, controls, gaps, regime_spans, session_regime, esc_from, afk_ns, ui_settle_ns,
          focus_settle_ns):
    cuts = [(a, a + focus_settle_ns, "focus_transition")] if focus_settle_ns else []
    for t, vk in ui_keys:
        if vk in SETTINGS_MENU_KEYS:
            cuts.append((t, t + ui_settle_ns, "settings_menu"))
        else:
            cuts.append((t, t + ui_settle_ns, "ui_key"))
...
def propose_segments(intervals, hud_samples, *, ui_keys=(), controls=None, gaps=(), regime_spans=(),
    ...
    cut: `[focus start, + focus_settle_ns)` after every focus regain, `[t_key, t_key + settle)` after a UI key
    (R1), everything after the first Esc (R3: proposed `after_settings_menu`, never acceptable), `afk_ns`
    ...
    ui_keys = sorted((int(t), int(vk)) for t, vk in ui_keys)
    _require(all(vk in UI_KEYS for _, vk in ui_keys), "ui_keys must be UI key packets")
    esc = [t for t, vk in ui_keys if vk in SETTINGS_MENU_KEYS]
    ...
    for a, b in intervals:
        ...
        cuts = _cuts(a, b, ui_keys=[k for k in ui_keys if a <= k[0] < b], controls=controls, gaps=gaps,
                     regime_spans=regime_spans, session_regime=session_regime,
                     esc_from=esc_from if esc_from is not None and esc_from < b else None,
                     afk_ns=afk_ns, ui_settle_ns=ui_settle_ns, focus_settle_ns=focus_settle_ns)
...
def check_freeze(folder, *, root, out_name="artifact-hashes.json"):
    ...
    bad = [k for k, v in {**doc["files"], **doc["external"]}.items()
           if not resolve(k).is_file() or sha256(resolve(k)) != v]
...
def step_table(dataset, *, stride_ns=STRIDE_NS, step_ns=None, regime="normal", regime_spans=(), game_patch=None,
               settings_hash=None, all_segments=True):
...
def load_cohort(paths, *, splits, declared_regimes, unseal=False):
...
FIT_ACTIONS = ("move_forward", "move_left", "move_back", "move_right", "jump", "web_swing", "get_over_here",
               "amazing_combo", "ultimate", "melee", "spider_power", "web_cluster", "team_up")
...
def write_steps(dataset, output, *, sitting, bindings, calibration, swing_mode, settings_hash, patch, regime,
                device_report, step_ns=STRIDE_NS, actions=FIT_ACTIONS, regime_spans=(), source=None, hud_layout="mk"):
    ...
    or `mouse:n`); presses of any other control are `unsupported`. A repeated make of a held key is not a press.
    """
    from bisect import bisect_right
    _require(list(bindings) == list(actions) and len(set(bindings.values())) == len(actions),
             "bindings must bind every action, in order, to distinct physical ids")
    ...
    event_times = [e.t_ns for e in dataset.events]
    by_pid = {pid: i for i, pid in enumerate(bindings.values())}
    ...
    header = dict(format=STEPS_FORMAT, session_id=p.session_id, media_sha256=dataset.media_sha256,
                  ...
                  settings_hash=settings_hash, patch=patch, source=source or {})
    ...
    for seg in review["segments"]:
        for left, right in dataset.intervals:
            ...
            while anchor + step_ns < hi:
                ...
                a, b = anchor, anchor + step_ns
                s0, s1 = state_at(a), state_at(b)
                h0 = [_hold(s0, pid) for pid in bindings.values()]
                h1 = [_hold(s1, pid) for pid in bindings.values()]
                press, release, unsupported = [0] * len(actions), [0] * len(actions), {}
                dx = dy = wv = wh = 0
                relative = True
                for i in range(bisect_right(event_times, a), bisect_right(event_times, b)):
                    e, before = dataset.events[i], (dataset.states[i - 1] if i else hd.HeldState())
                    row = e.payload
                    if e.type == "key":
                        pid = physical_id(row)
                        was = any(k.scan == row["scan"] and k.flags == row["flags"] & 6 for k in before.keys)
                        if pid in by_pid:
                            if row["down"] and not was and _hold(before, pid)[1]:
                                press[by_pid[pid]] += 1
                            elif not row["down"] and was:
                                release[by_pid[pid]] += 1
                        ...
                        for button in row["buttons_down"]:
                            pid = f"mouse:{button}"
                            if pid in by_pid:
                                if button not in before.mouse_buttons and before.observed:
                                    press[by_pid[pid]] += 1
                            else:
                                unsupported[pid] = unsupported.get(pid, 0) + 1
                        for button in row["buttons_up"]:
                            pid = f"mouse:{button}"
                            if pid in by_pid and button in before.mouse_buttons:
                                release[by_pid[pid]] += 1
                label = regime
                for rs, re_, rl in regime_spans:
                    if rs <= anchor < re_:
