...
from tests.human_intake_fixtures import (STEP, build_dataset, key_event, mouse_event, session_payload,
                                         steps_payload)
...
FIT_BINDINGS = {"move_forward": "key:17:0", "move_left": "key:30:0", "move_back": "key:31:0", "move_right": "key:32:0",
                "jump": "key:57:0", "web_swing": "key:42:0", "get_over_here": "key:33:0", "amazing_combo": "key:18:0",
                "ultimate": "key:16:0", "melee": "key:47:0", "spider_power": "mouse:1", "web_cluster": "mouse:2",
                "team_up": "key:46:0"}
DEVICES = dict(injected_control_packets=0, injected_zero_effect_packets=0)


def write_fit_steps(tmp_path, data, segs, name="s1.steps.jsonl", **kw):
    out = tmp_path / name
    args = dict(sitting="test-sitting", bindings=FIT_BINDINGS, settings_hash="h" * 64, patch="1.1/build",
                calibration={"yaw_deg_per_count": None, "pitch_deg_per_count": None, "source": "pending"},
                swing_mode={"automatic_swing": False, "hold_to_swing": True}, regime="normal", device_report=DEVICES,
                step_ns=100 * MS)
    args.update(kw)
    hi.write_steps(build_dataset(data, segs), out, **args)
...
def test_steps_v1_header_rows_and_reader_invariants(tmp_path):
    ...
    assert unsupported == {"mouse:4": 1}   # X1 only; the Esc lies past the last full step
...
def test_steps_v1_refuses_injected_input_and_partial_bindings(tmp_path):
    data = steps_payload(tmp_path)
    with pytest.raises(DemoError, match="injected"):
        write_fit_steps(tmp_path, data, two_segments(data), device_report=dict(DEVICES, injected_control_packets=1))
    with pytest.raises(DemoError, match="bind every action"):
        write_fit_steps(tmp_path, data, two_segments(data), name="b.jsonl",
                        bindings={k: v for k, v in FIT_BINDINGS.items() if k != "melee"})
