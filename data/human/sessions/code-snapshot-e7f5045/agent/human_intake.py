...
def load_denylist(path, *, sha256_pin=None):
    ...
    for row in doc["sessions"]:
        ...
        _require(row.get("allowed_split", "test") in SEALED_ALLOWED, "denylist allowed_split must be test or gate2")
        _require(row.get("allowed_split", "test") != "gate2" or isinstance(row.get("session_group"), str),
                 "a gate2 denylist row needs its session_group")
    return doc
...
REGISTRY_LISTS = ("sessions",) + hd.EXCLUDED_LISTS


def sealed_matches(row, denylist, base):
    """The denylist rows a registry row names, by session id, media path (resolved against `base`) or media sha256."""
    video = str((Path(base) / str(row.get("video_path", ""))).resolve()).lower()
    hashes = {row.get("expected_media_sha256"), row.get("media_sha256")} - {None}
    return [d for d in denylist["sessions"]
            if row.get("session_id") == d["session_id"] or d["media_sha256"] in hashes
            or (d.get("media_path") and video == str(Path(d["media_path"]).resolve()).lower())]


def check_registry(path, *, denylist):
    """Denylist first, then the raw registry rows, then the importer's own registry validation (R4).

    Every list of the registry is checked (review B1, 2026-09-26): a row naming a denylisted session id, media path or
    media sha256 may appear only in `sessions`, with exactly the split the denylist allows it (`allowed_split`, test by
    default) and, for gate2, in the denylist's session group (review B2). Sealed membership is the denylist's, so a
    registry that drops a sealed row and relabels its recording is refused.
    """
    path = Path(path)
    doc = json.loads(path.read_text(encoding="utf-8"))
    for name in REGISTRY_LISTS:
        for row in doc.get(name, []):
            for d in sealed_matches(row, denylist, path.parent):
                allowed = d.get("allowed_split", "test")
                _require(name == "sessions",
                         f"{row.get('session_id')}: denylisted session ({d['session_id']}) in {name}; sealed {allowed} only")
                _require(row.get("split") == allowed,
                         f"{row.get('session_id')}: denylisted session outside {allowed} ({d['session_id']})")
                if allowed == "gate2":
                    _require(row.get("session_group") == d["session_group"],
                             f"{row.get('session_id')}: gate2 session outside its pair {d['session_group']}")
...
def tally(rows, *, denylist=None, target_minutes=CORPUS_TARGET_MINUTES):
    ...
    for r in rows:
        ...
        _require(r["status"] != "sealed" or r.get("split") in SEALED_ALLOWED, "sealed rows must be test or gate2")
