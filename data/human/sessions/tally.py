...
def build(snapshot=DEFAULT_SNAPSHOT):
    """(tally doc, rendered markdown) from the live registry, the pinned denylist and every admitted freeze."""
    ...
    t.update(registry={"path": "data/human/session-splits.corpus.json", "sha256": sha(ROOT / "data/human/session-splits.corpus.json")},
             ...
             snapshot=snapshot, counted_rule="focused logged time ∩ accepted segments ∩ gap-free runs >= 1.6 s")
    ...
    return t, head + hi.render_tally(t) + tail
