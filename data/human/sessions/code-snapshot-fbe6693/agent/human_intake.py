...
CUT_ORDER = ("settings_menu", "after_settings_menu", "ui_key", "dead", "focus_transition", "afk", "timed_practice",
...
def timed_practice_span(start_reads, end_reads, interior=()):
    ...
    samples must never read range. The cut is conservative: from one ns after the last range frame before the round
    ...
    end_timed = next((t for t, label in e if label == "timed"), None)
    ...
    _require(not any(label == "timed" for t, label in e if t > after[0]), "end bracket: the banner flickers back")
...
def propose_segments(intervals, hud_samples, *, ui_keys=(), controls=None, gaps=(), regime_spans=(),
    ...
    without control-affecting input (R6), `timed_practice` spans (`[(start_ns, end_ns)]` from `timed_practice_span`
    ...
    for a, b in intervals:
        ...
        cuts = _cuts(a, b, ui=ui, controls=controls, gaps=gaps, regime_spans=regime_spans,
                     ...
                     timed=[(int(s), int(e)) for s, e in timed_practice])
